...
def Login(username, password):
    ...
    homepage = requests.get(
        url='http://www.renren.com/{}'.format(_Cookie['id']), headers=header).text
    requesttoken = re.search(
        r'requestToken\ \:\ \'(\-?\d{9})\'', homepage).group(1)
...
def Crawler(album_url, requesttoken, _rtk, Cookie):
    ...
    stdout('[*] 正在拉取所有照片链接\r')
    page = 1
    data_url = album_url.rstrip(
        'v7') + 'bypage/ajax/v7?page=1&pageSize=100&requestToken={}&_rtk={}'.format(requesttoken, _rtk)
    json_photolist = json.loads(requests.get(
        url=data_url, headers=header).text)
    ...
    while 100 == pagesize:
        page = page + 1
        data_url = album_url.rstrip(
            'v7') + 'bypage/ajax/v7?page={}&pageSize=100&requestToken={}&_rtk={}'.format(page, requesttoken, _rtk)
        json_photolist = json.loads(requests.get(
            url=data_url, headers=header, timeout=10).text)
        pagesize = len(json_photolist['photoList'])
        for photo in json_photolist['photoList']:
            photolist.append(photo['url'])
...
def Store(photolist, Cookie):
    ...
    for photo_url in photolist:
        ...
        photo_data = requests.get(
            url=photo_url, headers=headers, timeout=10).content
...
def Parse():
    parser = argparse.ArgumentParser(
        description='Example: crawler.py -u 12345@qq.com -p 123456 -a \\ http://photo.renren.com/photo/123456789/album-123456789/v7', add_help=False)
    option = parser.add_argument_group('Options')
    option.add_argument('-h', '--help', action='help',
                        help='Show Help Message And Exit')
    option.add_argument('-u', '--username', dest='username',
                        type=str, metavar='USERNAME', help='Specify RenRen Username')
    option.add_argument('-p', '--password', dest='password',
                        metavar='PASSWORD', help='Specify Corresponding Password')
    option.add_argument('-a', '--album', dest='album_url', metavar='ALBUM_URL',
                        help='Specify Album URL Of Yours Or Your Friends')
